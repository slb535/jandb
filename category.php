...
<div id="content">
	...
	<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
		<div class="post-single"><?php if ( has_post_thumbnail() ) { echo '<div class="archive-thumbnail">'; the_post_thumbnail('thumbnail'); echo '</div>'; } ?>
			 <h2 class="archive"><a href="<?php the_permalink() ?>" title="<?php the_title(); ?>" rel="bookmark"><?php the_title(); ?></a></h2>
                        <?php  $award_source = types_render_field ("award-source",  array('raw' => 'true'));
                                                if ( $award_source ) { echo '<span class="source">'.$award_source.'</span>'; }  ?>
